...
def main(page: ft.Page):
    ...
    # 버튼 눌렀을 때 실행할 함수들
    def on_summary_click(title):
        page.snack_bar = ft.SnackBar(ft.Text(f"{title} 버튼 클릭됨"))
        page.snack_bar.open = True
        page.update()

    def on_menu_click(title):
        page.snack_bar = ft.SnackBar(ft.Text(f"{title} 버튼 클릭됨"))
        page.snack_bar.open = True
        page.update()
    ...
    def summary_card(title):
        return ft.GestureDetector(
            ...
            on_tap=lambda e, t=title: on_summary_click(t),
    ...
    def menu_box(icon, title):
        return ft.GestureDetector(
            mouse_cursor=ft.MouseCursor.CLICK,
            on_tap=lambda e, t=title: on_menu_click(t),
            content=ft.Container(
                width=86,
                height=86,
                bgcolor=ft.Colors.WHITE,
